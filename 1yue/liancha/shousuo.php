...
$shousuo=$_POST['shousuo'];
mysql_connect('127.0.0.1','root','0509');
mysql_select_db('lianxi');
mysql_query('set names utf8');
$sql="select a_id,a_titile,type_name,a_important,a_is_show,a_author,a_author_email,a_content,a_desc from article inner join type on article.T_id=type.type_id where a_titile like '%$shousuo%'";
 		...
 		while ( $arr=mysql_fetch_assoc($res)) {
 			echo "<tr>";
 				...
 				if ($arr['a_important']==1) {
 					echo "<td><a href='update.php?id={$arr['a_id']}&&a_important=0'>普通</a></td>";
 				}else{
 					echo "<td><a href='update.php?id={$arr['a_id']}&&a_important=1'>置顶</a></td>";
 				}
 				echo "<td>".$arr['a_is_show']."</td>";
