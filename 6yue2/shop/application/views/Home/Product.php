...
<div class="top">
    ...
    <div class="i_car">
        ...
        <div class="car_bg">
            ...
            <div class="price_a"><a href="<?php echo site_url('Home/BuyCar/BuyCar') ?>">去购物车结算</a></div>
            <!--End 购物车已登录 End-->
        </div>
    </div>
</div>
...
<div class="menu_bg">
	<div class="menu">
    	<!--Begin 商品分类详情 Begin-->    
    	<?php $this->load->view('Home/common',array('category_type'=>$arr['category_type'])) ?> 
        <!--End 商品分类详情 End-->                                                     
    	<ul class="menu_r">
...
<div class="i_bg">
    ...
    <div class="content">
        ...
        <div id="tsShopContainer">
            <div id="tsImgS"><a href="images/p_big.jpg" title="Images" class="MagicZoom" id="MagicZoom"><img src="<?php echo base_url('uploads/goods/').$arr['goods']['goods_img'] ?>" width="390" height="390" /></a></div>
            <div id="tsPicContainer">
                <div id="tsImgSArrL" onclick="tsScrollArrLeft()"></div>
                <div id="tsImgSCon">
                    <ul>
                        <?php foreach ($arr['photo'] as $key => $v): ?>
                            <li onclick="showPic(0)" rel="MagicZoom" class="tsSelectImg"><img src="<?php echo base_url('uploads/goods/').$v['goods_img'] ?>" tsImgS="images/ps1.jpg" width="79" height="79" /></li>
                        <?php endforeach ?>
                    </ul>
                </div>
        ...
        <div class="pro_des">
        	<div class="des_name">
            	<p><?php echo $arr['goods']['goods_name'] ?></p>
                “开业巨惠，北京专柜直供”，不光低价，“真”才靠谱！
            </div>
            <div class="des_price">
            <input type="hidden" id="goods_price" value="<?php echo $arr['goods']['shop_price'] ?>" />
            	本店价格：<b>￥<span ><?php echo $arr['goods']['shop_price'] ?></span></b><br />
                库存：<span id="sku"><?php echo $arr['goods']['goods_brief'] ?></span>
            </div>
            <?php foreach ($arr['goods_attribute'] as $key => $value): ?>
                <div class="des_choice">
                    <span class="fl"><?php echo $value['attribute_name'] ?>：</span>
                    <ul>
                        <?php foreach ($value['attribute_value'] as $k => $v): ?>
                            <li class="attr_value" data-id="<?php echo $k ?>"><?php echo $v ?><div class="ch_img"></div></li>
                        <?php endforeach ?>
                    </ul>
                </div>
            <?php endforeach ?>
            ...
            <div class="des_join">
            	<div class="j_nums">
                    <input type="hidden" id="product_id" value="<?php echo $arr['product']['product_id'] ?>" />
                	<input type="text" value="1" id="goods_num" class="n_ipt" />
                    <input type="button" value="" onclick="addUpdate(jq(this));" class="n_btn_1" />
                    <input type="button" value="" onclick="jianUpdate(jq(this));" class="n_btn_2" />   
                </div>
                <span class="fl"><a onclick="cart_add()"><img src="images/j_car.png" /></a></span>
            </div>            
        </div>    
                  ...
                  <tr valign="top">
                    ...
                    <td>
                    	<span style="color:#3e3e3e; font-size:18px; font-weight:bold;">宝贝已成功添加到购物车</span><br />
                    	 &nbsp; &nbsp; 合计：<span id="price"></span>元
                    </td>
                  </tr>
                  <tr height="50" valign="bottom">
                  	<td>&nbsp;</td>
                    <td><a href="<?php echo site_url('Home/BuyCar/BuyCar/').$arr['goods_id'] ?>" class="b_sure">去购物车结算</a><a href="#" class="b_buy">继续购物</a></td>
                  </tr>
                </table>
...
<script>
auto_checked(<?php echo $arr['product']['product_sku'] ?>,<?php echo $arr['product']['product_price'] ?>,"<?php echo $arr['product']['product_list'] ?>")
    function auto_checked(sku,price,gsp_list){

        $('#price').html(price);
        $('#sku').html(sku);
        var gsp_id = gsp_list.split(',');
        
        for (var i = gsp_id.length - 1; i >= 0; i--) {
            $(".attr_value[data-id="+gsp_id[i]+"]").addClass('checked').siblings().removeClass('checked');
        }
    }
    function cart_add(){
        ...
        $.ajax({
            url: "<?php echo site_url('Home/Product/cart_add')?>",
            type: 'get',
            dataType: 'json',
            data: {product_id:product_id,goods_num:goods_num},
    ...
    $(document).on('click', '.attr_value', function() {
        $(this).addClass('checked').siblings().removeClass('checked');
        var obj=$(this);
        var goods_attribute_id=$(this).data('id');
        var checked_li=$(this).parents('.des_choice').siblings('.des_choice').find('.checked');
        
        var str=goods_attribute_id;
        for (var i = checked_li.length - 1; i >= 0; i--) {
            str+=','+checked_li.eq(i).data('id');
        }
        $.ajax({
            url: "<?php echo site_url('Home/Product/get_product')?>",
            type: 'get',
            dataType: 'json',
            data: {str: str,goods_attribute_id:goods_attribute_id},
            success:function(data){
                // console.log(data)
                if (data) {

                    $('#product_id').val(data[0].product_id);
                    auto_checked(data[0].product_sku,data[0].product_price,data[0].product_list);
                    //让规格列表的所有兄弟节点隐藏
                    // obj.parents('.des_choice').siblings('.des_choice').find('li').hide();
                    //跟点击的规格（蓝色）相关的规格显示出来
                    $.each(data, function(k, v){
                        $.each(v.product_list.split(','), function(kk, vv){
                            obj.parents('.des_choice').siblings('.des_choice').find('.attr_value[data-id="'+vv+'"]').show();
                        });
