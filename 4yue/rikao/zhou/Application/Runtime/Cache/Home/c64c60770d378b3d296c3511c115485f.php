...
<center>
	<thead>
		...
		<b>说说网</b> <span>&nbsp;&nbsp;&nbsp;关键字<input type="text" id="sousuo">&nbsp;<input type="button" onclick="sousuo()" value="搜索">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('Home/Infor/show');?>">首页</a>&nbsp;<a href="<?php echo U('Home/Inforshow/add');?>">发表说说</a>&nbsp;<?php echo $_SESSION['name'];?><a href="<?php echo U('Home/Infor/show');?>">(退出)</a></span>
		<hr width="500">
	</thead>
...
<script>
	...
	function seek(id){
		var xhr=new XMLHttpRequest();
		xhr.open('get', "<?php echo U('Home/Inforshow/seek');?>?id="+id)
		xhr.send()
		xhr.onreadystatechange=function (){
			if (xhr.readyState==4&&xhr.status==200) {
	...
	function sousuo(){
		...
		var xhr=new XMLHttpRequest();
		xhr.open('get', "<?php echo U('Home/Inforshow/ajax');?>?sousuo="+sousuo)
		xhr.send()
		xhr.onreadystatechange=function (){
			if (xhr.readyState==4&&xhr.status==200) {
				console.log(xhr.responseText)
