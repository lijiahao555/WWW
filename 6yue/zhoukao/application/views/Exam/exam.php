...
<body>
	<center>
	欢迎&nbsp;<font color="red"><?php echo $_SESSION['name'].$name['role_name'] ?></font>&nbsp;来到&nbsp;审核列表&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo site_url('Admin/login') ?>">退出</a>
	<br/>
		...
		<table border='1' id='tab'>
			...
			<?php foreach ($data as $key => $v): ?>
				<tr>
					...
					<td><a href="javascript:void(0);" class="add" data-id="<?php echo $v['infor_id'] ?>">通过</a>||<a href="<?php echo site_url('Exam/del') ?>?id=<?php echo $v['infor_id'] ?>">删除</a>||<a href="<?php echo site_url('Exam/up') ?>?id=<?php echo $v['infor_id'] ?>">修改</a></td>
				</tr>
			<?php endforeach ?>
		</table>
...
<script>
	$(document).on('click','.p',function(){
		...
		$.ajax({
		   type: "get",
		   url: "<?php echo site_url('Exam/exam_ajax') ?>",
		   data: {ye:ye},
		   dataType:'json',
	...
	$(document).on('click','.add',function(){
		...
		$.ajax({
		   type: "get",
		   url: "<?php echo site_url('Exam/exam_exam') ?>",
		   ...
		   success: function(msg){
				...
				for(k in msg){
					...
					<td>"+msg[k]['infor_name']+"</td>\
					<td><font color='red'>待审核</font></td>\
					<td><a href='javascript:void(0);'' class='add' data-id='"+msg[k]['infor_id']+"'>通过</a></td>\
				</tr>"
				}
				_tab+="</table>";
				$('#tab').html(_tab);
